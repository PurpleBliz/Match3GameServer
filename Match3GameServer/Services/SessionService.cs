...
public class SessionService(ILogger<SessionService> logger) : ISessionService
{
    ...
    public async Task AddPlayerAsync(WebSocket webSocket, int playerId)
    {
        if (_waitingPlayer == null)
        {
            ...
            logger.LogInformation("Player {PlayerId} is waiting for an opponent", playerId);
        }
        else
        {
            ...
            logger.LogInformation("Starting session {SessionId} with Player {WaitingPlayerId} and Player {PlayerId}",
